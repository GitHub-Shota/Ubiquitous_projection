...
int main()
{
    ...
    // 表示するウィンドウに名前を付ける
    cv::namedWindow("Level2_server", cv::WINDOW_NORMAL);
    // フルスクリーン表示
    cv::setWindowProperty("Level2_server", cv::WND_PROP_FULLSCREEN, cv::WINDOW_FULLSCREEN);

    // マウスイベントの登録
    cv::setMouseCallback("Level2_server", draw_line);

    // qが押されるまで表示を継続
    while (1)
    {
        cv::imshow("Level2_server", img);
